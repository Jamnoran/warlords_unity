...
namespace DunGen
{
	...
	public delegate bool TileMatchDelegate(Tile previousTile, GameObject previousPrefab, GameObject potentialNextPrefab, ref float weight);
		...
		private IEnumerable<DoorwayPair> GetPotentialDoorwayPairsForNonFirstTile()
		{
			foreach(var previousDoor in PreviousTile.Placement.UnusedDoorways)
			{
				...
				foreach(var tileWeight in TileWeights)
				{
					...
					if (IsTileAllowedPredicate != null && !IsTileAllowedPredicate(PreviousTile, PreviousPrefab, tileWeight.Value, ref weight))
						continue;
					...
					foreach (var nextDoor in nextTile.Doorways)
					{
						...
						if (IsValidDoorwayPairing(previousDoor, nextDoor, ref doorwayWeight))
